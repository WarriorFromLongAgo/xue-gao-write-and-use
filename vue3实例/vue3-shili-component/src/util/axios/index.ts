...
import {isNotEmpty} from "@/util/ObjectUtil";
import {useLoginStore} from "@/store/login/LoginInfoStore";
...
instance.interceptors.request.use(
    (config) => {
        ...
        // @ts-ignore
        if (isNotEmpty(config) && isNotEmpty(config.headers)) {
...
instance.interceptors.response.use(
    ...
    function (error) {
        ...
        if (response) {
            ...
            const store = useLoginStore();
            store.action.updateNetworkState;
